...
import { defineComponent, ref, computed, unref, createVNode, resolveDynamicComponent, withCtx, createTextVNode, useSSRContext } from 'vue';
...
const w = defineComponent({ __name: "products", __ssrInlineRender: true, setup(d) {
  ...
  ` }), m({ title: "Products - Metz Chicken Butchery", meta: [{ name: "description", content: "Browse our selection of premium free-range chicken products. From whole chickens to specialty cuts, we offer the finest quality chicken raised with care." }] }), (p, e, a, k) => {
    const u = W;
    e(`<div${ssrRenderAttrs(k)}><section class="relative h-[40vh] min-h-[300px] bg-gray-900 overflow-hidden"><div class="absolute inset-0"><img src="https://images.unsplash.com/photo-1587593810167-a84920ea0781?w=1920&amp;q=80" alt="Our premium chicken products" class="w-full h-full object-cover opacity-50"></div><div class="relative container mx-auto px-4 h-full flex items-center"><div class="max-w-2xl text-white"><h1 class="text-4xl md:text-5xl font-display font-bold mb-6">Our Products</h1><p class="text-xl text-gray-200">Premium quality chicken, raised with care and delivered fresh to your doorstep.</p></div></div></section><section class="py-20"><div class="container mx-auto px-4"><div class="mb-12"><div class="flex flex-wrap gap-4 justify-center"><!--[-->`), ssrRenderList(r, (t) => {
      e(`<button class="${ssrRenderClass(["px-6 py-2 rounded-full transition-colors", unref(i) === t.id ? "bg-primary-600 text-white" : "bg-gray-100 text-gray-700 hover:bg-gray-200"])}">${ssrInterpolate(t.name)}</button>`);
    }), e('<!--]--></div></div><div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8"><!--[-->'), ssrRenderList(unref(b), (t) => {
